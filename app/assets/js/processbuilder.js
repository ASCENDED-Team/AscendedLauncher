...
class ProcessBuilder {
  ...
  _lteMinorVersion(version) {
    return (
      Number(this.forgeData.id.split("-")[0].split(".")[1]) <= Number(version)
    );
  }
  ...
  _constructJVMArguments112(mods, tempNativePath) {
    ...
    if (process.platform === "darwin") {
      args.push("-Xdock:name=ASCENDEDLauncher");
      args.push(
  ...
  _constructJVMArguments113(mods, tempNativePath) {
        ...
        if (argDiscovery.test(args[i])) {
          ...
          switch (identifier) {
            ...
            case "natives_directory":
              ...
              break;
            case "launcher_name":
              val = args[i].replace(argDiscovery, "Ascended-Launcher");
              break;
    ...
    try {
      isAutoconnectBroken = ProcessBuilder.isAutoconnectBroken(
        this.forgeData.id.split("-")[2]
      );
    } catch (err) {
      logger.error(err);
      logger.error(
        "Forge version format changed.. assuming autoconnect works."
      );
